...
describe("Anomaly Controller", () => {
  ...
  it("should fetch all anomalies", async () => {
    ...
    expect(anomalyModel.getAnomalies).toHaveBeenCalledTimes(1);
  });

  it("should handle error when fetching all anomalies", async () => {
    (anomalyModel.getAnomalies as jest.Mock).mockRejectedValue(
  ...
  it("should create a new anomaly", async () => {
    ...
    expect(anomalyModel.createAnomaly).toHaveBeenCalledWith(newAnomaly);
    expect(anomalyModel.createAnomaly).toHaveBeenCalledTimes(1);
  });

  it("should handle error when creating a new anomaly", async () => {
    (anomalyModel.createAnomaly as jest.Mock).mockRejectedValue(
      new Error("Failed to create anomaly")
    );
  ...
  it("should fetch an anomaly by id", async () => {
    ...
    expect(anomalyModel.getAnomalyById).toHaveBeenCalledWith("1");
    expect(anomalyModel.getAnomalyById).toHaveBeenCalledTimes(1);
  });
  ...
  it("should handle error when fetching anomaly by id", async () => {
    (anomalyModel.getAnomalyById as jest.Mock).mockRejectedValue(
      new Error("Failed to fetch anomaly")
    );
  ...
  it("should save anomalies", async () => {
    ...
    const response = await request(app)
      .post("/api/anomalies/save")
      .send(anomalies);

    expect(response.status).toBe(201);
    expect(response.body).toEqual(savedAnomalies);
    expect(anomalyModel.createAnomaly).toHaveBeenCalledTimes(2);
    expect(anomalyModel.createAnomaly).toHaveBeenCalledWith(anomalies[0]);
    expect(anomalyModel.createAnomaly).toHaveBeenCalledWith(anomalies[1]);
  });

  it("should handle error when saving anomalies", async () => {
    (anomalyModel.createAnomaly as jest.Mock).mockRejectedValue(
      new Error("Failed to save anomalies")
    );
