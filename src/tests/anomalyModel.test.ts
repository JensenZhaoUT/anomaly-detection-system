...
describe('Anomaly Model', () => {
  ...
  it('should get all anomalies', async () => {
    const mockAnomalies: RowDataPacket[] = [{ id: 1, time: '2024-06-25T13:27:57.000Z', type: 'proximity_alert', message: 'Test message', frame: 'frame_1.png' } as RowDataPacket];
  ...
  it('should get anomaly by id', async () => {
    const mockAnomaly: RowDataPacket = { id: 1, time: '2024-06-25T13:27:57.000Z', type: 'proximity_alert', message: 'Test message', frame: 'frame_1.png' } as RowDataPacket;
    ...
    expect(pool.query).toHaveBeenCalledWith('SELECT * FROM anomalies WHERE id = ?', ['1']);
  });
});
