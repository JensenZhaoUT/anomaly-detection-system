...
describe("SearchPage", () => {
  ...
  it("should handle file upload", async () => {
    ...
    const input = screen.getByText("Upload Video")
  ...
  it("should filter anomalies based on search term", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Search by Time"), {
      target: { value: "13:27:57" },
    });
  ...
  it("should handle errors in anomaly fetching", async () => {
    ...
    expect(
      await screen.findByText("Failed to fetch anomalies")
    ).toBeInTheDocument(); // Ensure this error message is part of your UI
  });
});
