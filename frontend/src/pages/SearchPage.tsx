  ...
  const handleProcessVideo = async () => {
    ...
    for (let i = 0; i < detectedAnomalies.length; i += chunkSize) {
      ...
      try {
        ...
        setAnomalies((prevAnomalies) => [...prevAnomalies, ...response.data]); // Use response data
        setFilteredAnomalies((prevAnomalies) => [...prevAnomalies, ...response.data]); // Use response data
      } catch (error) {
