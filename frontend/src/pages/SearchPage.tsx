...
import { Container, Button, TextField, Table, TableBody, TableCell, TableHead, TableRow, Dialog, DialogTitle, DialogContent, DialogActions, DialogContentText } from '@mui/material';
...
const SearchPage: React.FC = () => {
  ...
  const processFrames = async (frames: HTMLImageElement[]): Promise<Anomaly[]> => {
    ...
    for (let i = 0; i < frames.length; i++) {
      ...
      try {
        ...
        if (anomalyDetected || Math.random() < 0.01) {
          const frameNumber = frame.dataset.frameNumber || (i + 1).toString();
          const newAnomaly: Anomaly = {
            id: anomalyCounter,
            time: moment().tz('America/Toronto').format(),  // Convert time to EST
  ...
  return (
    <Container>
        ...
        <Dialog open={!!selectedAnomaly} onClose={() => setSelectedAnomaly(null)}>
          ...
          <DialogContent>
            <div>
              ...
              <img src={`/static/images/${selectedAnomaly.frame}`} alt="Frame Image" />
            </div>
          </DialogContent>
          <DialogActions>
      ...
      <Button variant="contained" color="secondary" onClick={handleProcessVideo}>Process Video</Button>
      {videoFile && (
        <div>
          <h2>Video</h2>
